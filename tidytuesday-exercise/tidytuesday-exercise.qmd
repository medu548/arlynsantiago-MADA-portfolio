...
eclipse_duration <- eclipse_total_2024 %>%
   mutate(time_difference = as.numeric(eclipse_4 - eclipse_3)) %>% #had to convert to numeric for modeling
  group_by(name) %>%
...
model_data <- eclipse_total_2024 %>% select(lat,lon, eclipse_duration)

# split dataset into 70% training and 30% test
set.seed(1234) # using set from Module 11
data_split <- initial_split(model_data, prop = 0.7)
train_data <- training(data_split)
test_data <- testing(data_split)

# Creating recipe to use for all models
recipe <- recipe(eclipse_duration ~ lat +lon, data = train_data) %>% 
